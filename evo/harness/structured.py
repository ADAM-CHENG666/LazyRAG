...
_REPAIR_SYSTEM_PROMPT = (
    ...
    '(which contains a previous bad model output, validation errors, and the '
...
def _repair_user(original_user: str, raw: str, errors: list[str], schema: dict) -> str:
    ...
    return (
        ...
        '## YOUR JOB\n'
        'Emit ONE JSON object satisfying the shape above. Do NOT echo the bad '
        'output. Do NOT call any tool. Do NOT wrap in markdown. Output JSON only.'
    )


def _dump_raw(session: AnalysisSession, agent: str, raw: str) -> str | None:
    try:
        path = (session.config.storage.runs_dir / session.run_id
...
def _call_llm(session: AnalysisSession, *, producer: Callable[[], str],
              cache_key: str | None, agent: str) -> str:
    return session.llm.call(producer=producer, cache_key=cache_key,
                             use_cache=cache_key is not None, agent=agent)
...
def invoke_structured(session: AnalysisSession, invoker: LLMInvoker, user_text: str,
    ...
    for attempt in range(max_repair + 1):
        ...
        if attempt == 0:
            call = (lambda u=current_user: producer(u)) if producer is not None \
                   else (lambda u=current_user: invoker.invoke(u))
            raw = _call_llm(session, producer=call, cache_key=cache_key, agent=agent_tag)
        else:
            raw = _call_llm(
                session,
                producer=lambda u=current_user: invoker.invoke(
                    u, system_prompt=_REPAIR_SYSTEM_PROMPT),
                cache_key=None, agent=agent_tag,
            )
    ...
    session.telemetry.emit(
        'schema_repair_failed', agent=agent,
        errors=errors[:10], raw_preview=raw_last[:500], raw_path=raw_path,
    ...
    if session.schema_failure_count >= _MAX_SCHEMA_FAILURES:
        raise ApplyError(
            ...
            (f'model failed to follow JSON schema {session.schema_failure_count} '
             f'times across agents; this model likely cannot drive evo. '
             'Switch evo_llm to qwen3-max / deepseek-v3 / claude-sonnet.'),
            {'failures': session.schema_failure_count, 'agent': agent,
