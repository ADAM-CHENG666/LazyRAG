...
func selectSlotRevision(ctx context.Context, db *gorm.DB,
	...
	err := common.TransactionWithSQLiteBusyRetry(ctx, db, func(tx *gorm.DB) error {
		...
		if err := tx.Model(&orm.WorkflowSlotRevision{}).Where("session_id = ? AND slot_id = ? AND selected = ?", sessionID, slotID, true).Pluck("id", &selectedIDs).Error; err != nil {
			return err
		}
		if err := artifactgraph.CheckConsumers(ctx, tx, sessionID, selectedIDs...); err != nil {
			return err
		}
...
func UpdateSelectedHumanArtifactValue(
	...
	err := common.TransactionWithSQLiteBusyRetry(ctx, db, func(tx *gorm.DB) error {
		...
		requiresCopyOnWrite := selected.ChangeSource != "human" || sealed
...
func SaveHumanArtifactValue(ctx context.Context, db *gorm.DB,
	...
	err := common.TransactionWithSQLiteBusyRetry(ctx, db, func(tx *gorm.DB) error {
		...
		version = 0
		inPlace = false
		// A checkpoint is an explicit save after the panel's invalidation
		// confirmation. Background drafts retain the live-consumer guard.
		if !draft {
