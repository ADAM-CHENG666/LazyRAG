...
func createTestConversation(t *testing.T, repo *Repository, id, owner string) {
	t.Helper()
	if err := repo.db.AutoMigrate(&orm.Conversation{}, &orm.WorkflowSession{}, &orm.WorkflowSessionStep{}, &orm.TaskCenterTask{}); err != nil {
		t.Fatal(err)
	}
	now := time.Now().UTC()
...
func TestDeleteArtifactCreatesTombstoneAndPreservesHistory(t *testing.T) {
	...
	now := time.Now().UTC()
	if err := repo.db.AutoMigrate(&orm.WorkflowSession{}, &orm.WorkflowSessionStep{}, &orm.WorkflowHumanArtifact{},
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			if err := repo.db.AutoMigrate(
				&orm.WorkflowSession{}, &orm.WorkflowSessionStep{}, &orm.WorkflowHumanArtifact{}, &orm.WorkflowSlotRevision{},
...
func TestFacadeArtifactMutationSerializesWithConcurrentConsumerBinding(t *testing.T) {
	for _, operation := range []string{"patch", "delete"} {
		t.Run(operation, func(t *testing.T) {
			models := []any{
				&orm.WorkflowEvent{}, &orm.WorkflowSession{}, &orm.WorkflowSessionStep{}, &orm.WorkflowHumanArtifact{}, &orm.WorkflowSlotRevision{},
				&orm.WorkflowSessionStep{}, &orm.WorkflowAttemptInputBinding{}, &orm.WorkflowRouteDecision{},
			}
