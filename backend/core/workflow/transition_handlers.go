...
func applyWorkflowTransition(ctx context.Context, tx *gorm.DB, sessionID string, req transitionCommandRequest) (transitionCommandResponse, orm.WorkflowSession, []string, error) {
	...
	err = func() error {
		...
		now := time.Now().UTC()
		packageFuncs := loadPackageScriptFunctions(ctx, tx, session.WorkflowRevisionID)
		responseTasks := make([]transitionTaskResponse, 0, len(targets))
		for _, target := range targets {
			handOff := req.HandOff
			nodeDef := graph.Nodes[target.TargetStepID]
			taskID := target.TaskID
			// External-agent sessions keep package-script steps on the host.
			// A step that declares any tool outside tool_scripts runs inside LazyMind.
			executorHost := executorHostForStep(session.ControllerHost, nodeDef, packageFuncs)
			if executorHost == "external-agent" {
				...
				_, taskID, _, launchErr = launchWorkflowAttempt(ctx, tx, store.State(), session.ConversationID, session.TriggerHistoryID, session.CreateUserID, target.TaskID, session.WorkflowID+":"+target.TargetStepID, stepObjective, params, inputKeys, nodeDef.Outputs, req.LLMConfig, req.ToolConfig, false, false)
				if launchErr != nil {
					return launchErr
				}
			}
...
func invalidateForOperation(ctx context.Context, tx *gorm.DB, session *orm.WorkflowSession, graph *graphengine.CompiledStateGraph, commandID, operation, target string) error {
	...
	if controlstore.Controlled(*session) {
		return pruneControlledListOrders(tx, session.ID)
	}
	return nil
}
