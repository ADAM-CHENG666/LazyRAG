...
function renderActions(options?: { dirty?: boolean; stepStatus?: string; control?: WorkflowControlView }) {
  ...
  render(<WorkflowControlActions
    ...
    context={{
      ...
      tab: { id: 'script', label: 'Script', step_id: 'script', slots: [] },
      pending: false,
...
describe('WorkflowControlActions review footer', () => {
  it('shows review and regeneration actions without stop while awaiting review', () => {
    renderActions();
    expect(screen.getByRole('button', { name: 'chat.workflowContinueExecution' })).toBeInTheDocument();
    ...
    expect(screen.getByRole('button', { name: 'chat.workflowSkipThisApproval' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'chat.workflowSkipFollowingApprovals' })).toBeInTheDocument();
  });
  ...
  it('falls back to confirm-only when the host cannot continue from the panel', () => {
    ...
    expect(screen.queryByRole('button', { name: 'chat.workflowContinueExecution' })).not.toBeInTheDocument();
  });
  ...
  ] as const)('routes shared approval choice %s through the external command', (label, scope) => {
    const { act, runAction } = renderActions({ dirty: true });
    fireEvent.click(screen.getByRole('button', { name: label }));
    expect(runAction).toHaveBeenCalledWith(expect.any(Function), true);
  ...
  it('disables every approval choice while host delivery is pending', () => {
    const { act } = renderActions({ control: control({
      delivery: { id: 'action', kind: 'continue', status: 'dispatching', execution_id: '', last_error: '' },
    }) });
    for (const label of ['chat.workflowContinueExecution', 'chat.workflowSkipThisApproval', 'chat.workflowSkipFollowingApprovals']) {
