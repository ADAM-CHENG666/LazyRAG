...
const fixture = vi.hoisted(() => ({
  ...
  setFocusedTab: vi.fn(),
  control: vi.fn(async () => ({})),
  getSession: vi.fn(async () => ({ data: { data: { state_version: 7 } } })),
}));
...
vi.mock('@/modules/chat/utils/request', () => ({ WorkflowSessionApi: () => ({ setApprovalPreference: fixture.setApprovalPreference, control: fixture.control, getSession: fixture.getSession }) }));
...
beforeEach(async () => {
  ...
  fixture.session = {
    ...
    steps: ['prepare', 'write_document', 'stale_step'].map((step_id, index) => ({
      id: step_id, session_id: 'layout-test', step_id, attempt: 1, task_id: '', status: 'succeeded',
      validity: step_id === 'stale_step' ? 'stale' : 'effective', created_at: `2026-09-16T00:00:0${index}Z`, updated_at: '',
    })),
...
describe('shared workflow compact layout', () => {
  it('regenerates the selected completed step after confirmation and saving', async () => {
    const send = vi.fn();
    ...
    fireEvent.click(within(await screen.findByRole('tooltip')).getByRole('button', { name: '重新生成' }));
    await waitFor(() => expect(fixture.control).toHaveBeenCalledWith('layout-test', expect.objectContaining({ kind: 'rewind', step_id: 'write_document', expected_state_version: 7 })));
  ...
  it('keeps regeneration available for completed workflows without document footer actions', async () => {
    ...
    expect(within(footer).getByRole('button', { name: '重新生成' })).toBeVisible();
  });
  ...
  it('retains only the relevant running and failed workflow controls', async () => {
    ...
    fixture.session = { ...fixture.session, status: 'failed', steps: fixture.session.steps?.map(step => step.step_id === 'write_document' ? { ...step, status: 'failed' } : step) };
