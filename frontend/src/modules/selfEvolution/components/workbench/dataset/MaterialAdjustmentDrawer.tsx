...
export function MaterialAdjustmentDrawer({
  ...
  const error = optionsError || documentsError;

  const loadDocuments = useCallback(async () => {
    setLoading(true);
    setDocumentsError(undefined);
    try {
      // The document scope must cover the whole thread, not the filtered page
      // currently shown in the material list, or applying would silently
      // exclude documents the user never saw.
      const rows = await loadAllDocuments(root);
      setAllDocuments(rows);
      setDocuments(Object.fromEntries(rows.map((item) => [documentKey(item), item.included])));
    } catch (caught) {
      setAllDocuments(undefined);
      setDocumentsError(describeRequestError(caught, "读取文档列表失败"));
    } finally {
      setLoading(false);
    }
  }, [root]);
  ...
  }, [allDocuments, options?.knowledge_bases]);
  ...
  const changes = useMemo(() => {
    ...
    const changedDocuments = (allDocuments || [])
      .filter((item) => Boolean(documents[documentKey(item)]) !== item.included)
      .map((item) => ({
        knowledge_base_id: item.knowledge_base.id,
        document_id: item.document_id,
        included: Boolean(documents[documentKey(item)]),
      }));
    ...
    return payload;
  }, [allDocuments, documents, knowledgeBases, options, selectedLayoutIds, selectedRuleIds, target]);
                ...
                <div className="dataset-source-group" key={group.id}>
                  ...
                  <div className="dataset-check-list">
                    {group.documents.length ? (
                      ...
                      <span className="dataset-note">该知识库暂无已发现的文档。</span>
                    )}
                  </div>
                </div>
              ))
...
async function loadAllDocuments(root: string): Promise<DocumentRow[]> {
  const items: DocumentRow[] = [];
  let pageToken: string | undefined;
  for (let page = 0; page < MAX_DOCUMENT_PAGES; page += 1) {
    const response = await getJson<PagedResponse<DocumentRow>>(`${root}/materials/documents`, {
