...
const workflowInstructions = "For a selected LazyMind Workflow, call workflow.start. Immediately after a successful start, automatically open the returned interaction_url in the Codex built-in browser using mcp__codex_app__open_in_codex (open_in_codex), with target.type=browser, target.url=interaction_url, and placement=bottom. Discover the host tool if needed. Do not ask the user to click a link. Open once per new session, not on each state read. If the host browser tool is unavailable or fails, provide the returned link and explain the limitation; do not claim the panel opened. Then call workflow.step.begin for a ready step according to state. Opening the browser does not itself advance steps or guarantee automatic agent wakeup after user interaction. When the user asks to continue, read workflow.state first; claim an existing active execution with workflow.step.claim instead of beginning a duplicate. Execute a step_contract only with an execution_handle; publish each output with workflow.artifact.publish as it is generated, then send only the outcome with workflow.step.complete using that handle unchanged. When executor_host is lazymind, wait for state updates. In external steps, save_artifact/save_artifacts means publish each output immediately (key becomes slot) with a stable positive seq per slot, wait for its acknowledgement, and complete only when execution finishes. Read inputs from step_contract.inputs; resolve references with workflow.artifact.get or workflow.input.get, and use workflow.artifact.list to find an artifact ID by slot key. Follow control.continuation to continue or wait. Report completion only when workflow.state confirms completed, then deliver artifact links from workflow.artifact.list."
