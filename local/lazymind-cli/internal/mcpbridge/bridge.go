...
const workflowInstructions = "For a selected LazyMind Workflow, call workflow.start, then workflow.step.begin for a ready step according to state. Execute a step_contract only with an execution_handle; submit the outcome and required outputs with workflow.step.submit using that handle unchanged. When executor_host is lazymind, wait for state updates. In external steps, save_artifact/save_artifacts means collect submit.outputs (key becomes slot) and submit together when the step finishes. Read inputs from step_contract.inputs; resolve references with workflow.artifact.get or workflow.input.get, and use workflow.artifact.list to find an artifact ID by slot key. Follow control.continuation to continue or wait. Report completion only when workflow.state confirms completed, then deliver artifact links from workflow.artifact.list."
